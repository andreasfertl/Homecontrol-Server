...
namespace RESTAPI {
	struct command {
	public:
		command(const utility::string_t& command, const utility::string_t& value, const std::function<std::optional<web::json::value>(const utility::string_t& value)>& callback) :
			m_Command(command),
			m_Value(value),
			m_Callback(callback)
		{
